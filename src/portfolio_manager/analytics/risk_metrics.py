"""
Comprehensive risk metrics module containing all functions from the original risk.py file.

This module implements the complete EDHEC risk toolkit with modern portfolio theory functions,
Monte Carlo simulations, bond analytics, and CPPI strategies.
"""
...
# Update data path to be relative to package
...
def get_ffme_returns():
    ...
    """
    me_m = pd.read_csv(DATA_PATH / "Portfolios_Formed_on_ME_monthly_EW.csv",
...
def get_hfi_returns():
    ...
    """
    hfi = pd.read_csv(DATA_PATH / "edhec-hedgefundindices.csv",
...
def get_ind_file(filetype):
    ...
    if filetype == "returns":
        ...
        name = "size"
        divisor = 1
    ind = pd.read_csv(DATA_PATH / f"ind30_m_{name}.csv", header=0, index_col=0)/divisor

...
asset_universe = {
    ...
    "Market ETFs": {"SPY": "SPDR S&P 500 ETF", "QQQ": "Invesco QQQ Trust"},
    "Fixed Income": {
    ...
    "International": {"VEA": "Vanguard FTSE Developed Markets ETF"},
}
...
print(
    f"Asset Universe: {len(all_symbols)} instruments across {len(asset_universe)} asset classes"
)
...
for symbol, weight in strategic_weights.items():
    if symbol in assets:
        strategic_portfolio.add_asset(
            symbol, assets[symbol], weight, price_data=asset_data.get(symbol)
        )
...
try:
    ...
    try:
        ...
        for symbol, contrib in sorted(
            risk_contrib.items(), key=lambda x: x[1], reverse=True
        ):
...
if "portfolio_returns" in locals() and not portfolio_returns.empty:
    ...
    for conf in confidence_levels:
        ...
        print(
            f"{conf:>8.0%}      {hist_var:>9.2%}  {hist_cvar:>9.2%}  {gauss_var:>10.2%}"
        )
...
if len(returns_data) >= 4:  # Need at least 4 assets for meaningful efficient frontier
    ...
    if not returns_df.empty:
        ...
        try:
            ...
            n_points = 50
            weights_list = optimal_weights(
                n_points, expected_returns.values, cov_matrix.values
            )
            ...
            msr_weights = msr(
                0.02, expected_returns.values, cov_matrix.values
            )  # 2% risk-free rate
            ...
            for i, symbol in enumerate(returns_df.columns):
                ...
                plt.scatter(
                    asset_vol, asset_ret, marker="s", s=80, alpha=0.7, label=symbol
                )
...
optimization_assets = [
    assets[symbol] for symbol in optimization_symbols if symbol in assets
]
...
if "portfolio_returns" in locals() and not portfolio_returns.empty:
    ...
    for i in range(min(50, mc_results_df.shape[1])):
        plt.plot(
            mc_results_df.index, mc_results_df.iloc[:, i], alpha=0.3, linewidth=0.5
        )
    plt.plot(
        mc_results_df.index,
        mc_results_df.mean(axis=1),
        "r-",
        linewidth=2,
        label="Mean Path",
    )
    ...
    plt.axvline(
        final_returns.mean(), color="red", linestyle="--", linewidth=2, label="Mean"
    )
    plt.axvline(
...
if "portfolio_returns" in locals() and not portfolio_returns.empty:
    ...
    cppi_results = run_cppi(
        risky_r=portfolio_returns, m=3, start=1000, floor=0.8, riskfree_rate=0.03
    )
    ...
    plt.subplot(2, 2, 1)
    plt.plot(
        cppi_results["Wealth"].index,
        cppi_results["Wealth"].iloc[:, 0],
        label="CPPI Wealth",
        linewidth=2,
    )
    plt.plot(
        cppi_results["Risky Wealth"].index,
        cppi_results["Risky Wealth"].iloc[:, 0],
        label="Risky Asset Only",
        linewidth=2,
    )
    plt.plot(
        cppi_results["floor"].index,
        cppi_results["floor"].iloc[:, 0],
        "r--",
        label="Floor",
        linewidth=1,
    )
    ...
    plt.subplot(2, 2, 2)
    plt.plot(
        cppi_results["Risky Allocation"].index,
    ...
    plt.subplot(2, 2, 3)
    plt.plot(
        cppi_results["Risk Budget"].index, cppi_results["Risk Budget"].iloc[:, 0] * 100
    )
    ...
    plt.plot(
        drawdown_cppi.index,
        drawdown_cppi["Drawdown"] * 100,
        label="CPPI Drawdown",
        linewidth=2,
    )
    plt.plot(
        drawdown_risky.index,
        drawdown_risky["Drawdown"] * 100,
        label="Risky Drawdown",
        linewidth=2,
    )
...
if asset_returns:
    ...
    if not returns_df.empty:
        ...
        print(
            f"{'Asset':<8} {'Weight':<8} {'Return':<10} {'Vol':<8} {'Sharpe':<8} {'Contrib':<10}"
        )
        print("-" * 65)

        for symbol, metrics in sorted(
            individual_stats.items(), key=lambda x: x[1]["contribution"], reverse=True
        ):
            print(
                f"{symbol:<8} {metrics['weight']:<7.1%} {metrics['ann_return']:<9.2%} "
        ...
        for symbol in symbols:
            plt.scatter(
                individual_stats[symbol]["ann_vol"],
...
features = {
    ...
    "Monte Carlo": {"Package GBM": "mc_results" in locals()},
    "CPPI Strategy": {"Package CPPI": "cppi_results" in locals()},
    "Summary": {"Package Summary Stats": "summary" in locals()},
    "Visualizations": {
